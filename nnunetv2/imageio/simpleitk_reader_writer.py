...
class SimpleITKIO(BaseReaderWriter):
    ...
    def process_read_images(self, itk_images) -> Tuple[np.ndarray, dict]:
        images = []
        spacings = []
        origins = []
        directions = []
    ...
    def write_image(self, img: np.ndarray, output_fname: str, properties: dict) -> None:
        assert img.ndim == 3, 'images must be 3d. If you are exporting a 2d segmentation, please provide it as shape 1,x,y'
        output_dimension = len(properties['sitk_stuff']['spacing'])
        assert 1 < output_dimension < 4
        if output_dimension == 2:
            img = img[0]
