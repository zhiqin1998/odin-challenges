...
def save_checkpoint(args, epoch, model, max_accuracy, optimizer, lr_scheduler, logger, best_model=False, save_limit=-1):
    save_state = {
    ...
    if best_model:
        ...
        save_path = os.path.join(args.output, f"ckpt_epoch_{epoch:04d}.pth")
    ...
    if save_limit > 0:
        ...
        if len(ckpts) > save_limit:
            ckpts = ckpts[:-save_limit]
            for ckpt in ckpts:
                os.remove(os.path.join(args.output, ckpt))
...
def block_wise_mask(input_size, n=1, mask_patch_size=16, model_patch_size=8, mask_ratio=0.3):
    rand_size = [n] + [x // mask_patch_size for x in input_size]
    scale = mask_patch_size // model_patch_size
    token_count = np.prod(rand_size)
    ...
    mask[mask_idx] = 1
    mask = mask.reshape(rand_size)
    mask = mask.repeat(scale * model_patch_size, axis=1).repeat(scale * model_patch_size, axis=2).repeat(scale * model_patch_size, axis=3)
