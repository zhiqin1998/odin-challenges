...
class SimMIMSkip(nn.Module):
    ...
    def forward(self, x, mask, x_org, cl_type):
        ...
        # disabled because single modal only
        ...
        if self.loss == "mask_only":
            ...
            loss_recon = F.mse_loss(x_org, x_rec, reduction="mean")
            _, loss_var_l2 = self.get_image_prior_losses(x_rec)
            bs = x.shape[0]
            img_l2_loss = torch.norm(x_rec.view(bs, -1), dim=1).mean()
            loss_recon = loss_recon + 0.0001 * loss_var_l2 + 1e-5 * img_l2_loss

        loss_cont = torch.tensor(0, dtype=loss_recon.dtype, device=loss_recon.device) # disabled mm cont loss
...
class nnUNet(nn.Module):
    ...
    def forward(self, x, mask, x_org, cl_type):
        ...
        if self.loss == "mask_only":
            ...
            bs = x.shape[0]
            img_l2_loss = torch.norm(x_rec.view(bs, -1), dim=1).mean()
            loss_recon = loss + 0.0001 * loss_var_l2 + 1e-5 * img_l2_loss

        return loss_recon, torch.tensor(0., device=loss_recon.device, dtype=loss_recon.dtype), x_rec, mask
...
def build_simmim(args):
    ...
    if model_type == "swin":
        ...
        with open(args.nnunet_plan, 'r') as f:
            plans = json.load(f)
        plans_manager = PlansManager(plans)
        configuration_manager = plans_manager.get_configuration(args.nnunet_conf)

        encoder = get_network_from_plans(
        ...
        model = nnUNet(encoder=encoder, loss=args.loss_type, img_size=(args.roi_x, args.roi_y, args.roi_z),
                       in_channels=args.in_channels, choice=args.choice)
